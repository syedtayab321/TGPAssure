...
class HomePage(QWidget):
    ...
    magnetic_requested = Signal()
    gravity_requested = Signal()
    ...
    def __init__(self, parent=None):
        ...
        text = QVBoxLayout()
        h = QLabel("TGPAssure")
        h.setObjectName("homeHeading")
        s = QLabel("Integrated Geophysical QA/QC • Processing • Visualization • Decision Support")
        s.setObjectName("homeSubheading")
        text.addWidget(h)
        text.addWidget(s)
        bl.addLayout(text, 1)
        for lab, sig, icon in (
            ("New Project", self.new_project_requested, "document-new"),
            ("Open Project", self.open_project_requested, "document-open"),
            ("Import Data", self.import_requested, "document-import"),
            ("Subscription", self.subscription_requested, "view-statistics"),
            ("Logout", self.logout_requested, "system-log-out"),
        ):
            b = QToolButton()
            b.setObjectName("homeBannerAction")
            b.setToolButtonStyle(Qt.ToolButtonTextBesideIcon)
            b.setText(lab)
            b.setIcon(get_icon(icon, "#FFFFFF", 22))
            b.setIconSize(QSize(22, 22))
            b.clicked.connect(sig.emit)
            bl.addWidget(b)
        root.addWidget(banner)

        title = QLabel("Core Workspaces")
        ...
        for i, (name, desc, module, icon, color) in enumerate(cards):
            ...
            card.setIcon(get_icon(icon, color, 38))
            card.setIconSize(QSize(38, 38))
            card.setText(name + "\n" + desc)
            card.setToolTip(desc)
            card.setMinimumHeight(118)
            card.setCursor(Qt.PointingHandCursor)
            card.clicked.connect(lambda checked=False, m=module: self.module_requested.emit(m))
            grid.addWidget(card, i // 4, i % 4)
        root.addLayout(grid)

        strip = QFrame()
        strip.setObjectName("homeWorkflowStrip")
        sl = QHBoxLayout(strip)
        sl.setContentsMargins(16, 10, 16, 10)
        for t in ("1  Import & Index", "2  Validate & QC", "3  Process", "4  Visualize & Interpret", "5  Report & Audit"):
            label = QLabel(t)
            label.setObjectName("homeWorkflowStep")
            label.setAlignment(Qt.AlignCenter)
            sl.addWidget(label, 1)
